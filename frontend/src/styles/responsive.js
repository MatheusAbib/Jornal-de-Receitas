...
@media (max-width: 480px) {
  ...
  .newspaper-price { position: relative; top: 0; left: 0; right: 0; margin: 10px auto; width: fit-content; }
  .menu-icon { font-size: 1.3rem; height: 30px; width: 30px; }
  .carousel-caption h3 { font-size: 1.5rem; }
  .carousel-caption p { font-size: 0.9rem; padding: 0 10px; }
  .carousel-control { width: 40px; height: 40px; font-size: 1.2rem; }
  .filter-group input { padding: 10px 10px 10px 40px; }
  .filter-buttons { margin-top: 0; gap: 0; }
  .recipe-filters { padding: 15px; margin: 20px 0 0 0; }
  .recipe-filters h3 { font-size: 1.3rem; }
  .receitas-grid { grid-template-columns: 1fr !important; gap: 20px; margin: 30px 0; }
  .card { margin-bottom: 15px; }
  .card-image { height: 180px; }
  .card-content { padding: 15px; }
  .card-titulo { font-size: 1.4rem; }
  ...
  .user-view-info { grid-template-columns: 1fr; gap: 0; }
  .form-group input { font-size: 0.9rem; padding: 10px; }
  .modal-content { width: 100% !important; max-width: 100% !important; margin: 10px auto !important; border-radius: 8px !important; }
...
@media (min-width: 481px) and (max-width: 768px) {
  ...
  .receitas-grid { grid-template-columns: repeat(2, 1fr) !important; gap: 25px; }
  .recipe-filters { padding: 20px; }
  .modal-content { width: 90% !important; max-width: 500px !important; }
}

@media (min-width: 769px) and (max-width: 1024px) {
  .newspaper-title { font-size: 2rem; }
  .newspaper-date { display: none; }
  .newspaper-price { display: none; }
  .newspaper-title::before, .newspaper-title::after { display: none; }
  .header-user-profile, .header-logout-btn { padding: 4px 14px; }
  .receitas-grid { grid-template-columns: repeat(2, 1fr) !important; }
...
@media (min-width: 900px) and (max-width: 1200px) {
  .nova-form-row { gap: 0; display: flex; flex-direction: column; }
  .newspaper-title { font-size: 3rem; }
  .receitas-grid { grid-template-columns: repeat(3, 1fr) !important; }
}
...
.sobre-container{        padding: 80px 8px 40px;
        margin: 0;}

  .tab-button .badge { padding: 2px 6px; font-size: 0.6rem; }
  .tab-buttons { display: flex; align-items: stretch; flex-direction: row; flex-wrap: nowrap;     margin: 38px auto 0px auto;}
  .tab-button { text-align: center; font-size: 0.7rem; gap: 3px; }
  .recipe-count { display: none; }
  .modal-actions { flex-direction: column; gap: 0; }
  .modal-actions button { width: 100%; margin-bottom: 10px; }
  .card-meta { flex-wrap: wrap; gap: 10px; }
  .carousel-controls { padding: 0 5px; }
  ...
  .modal-overlay { overflow-y: auto !important; -webkit-overflow-scrolling: touch !important; padding: 8px }
  body { max-width: 100% !important; overflow-x: hidden !important; padding: 0 !important; }
  .header-full-width { width: 100% !important; left: 0 !important; right: 0 !important; margin-left: 0 !important; margin-right: 0 !important; }
}
...
@media print {
  .header-full-width, .carousel-container, .modal-overlay, .modal-content, .admin-sidebar, .menu-icon, .card-favorite-btn, .carousel-controls, .social-links, .footer-bottom { display: none !important; }
  body { background: white; color: black; }
  .card, .minha-card, .admin-recipe-card { break-inside: avoid; box-shadow: none; border: 1px solid #ddd; }
}

@media (max-width: 768px) {
  .minhas-tabs { gap: 0; margin: 0 0 20px; }
  .minhas-tab { padding: 10px 14px; font-size: 0.7rem; flex: 1; justify-content: center; }
  .minhas-grid { grid-template-columns: 1fr 1fr !important; gap: 10px; }
  ...
  .minha-info { padding: 12px 12px 14px; }
...
@media (max-width: 768px) {
  ...
  .carousel-caption h3 { font-size: 1.4rem; letter-spacing: 0.5px; }
  .carousel-caption p { font-size: 0.9rem; margin: 6px auto; }
  .carousel-control { width: 42px; height: 42px; font-size: 1rem; }
  .carousel-controls { padding: 0 12px; }
  .carousel-indicators { bottom: 14px; gap: 8px; }
  .carousel-indicator { width: 24px; height: 5px; }
...
@media (max-width: 768px) {
  ...
  .card-meta { gap: 6px; margin-bottom: 0; padding-bottom: 6px; }
  .card-meta-row { gap: 8px; }
  .card-meta-item { font-size: 0.72rem; gap: 4px; }
  .card-meta-item i { font-size: 0.7rem; }
  .card-link { font-size: 0.68rem; padding: 8px 10px; letter-spacing: 0.5px; }
...
@media (max-width: 480px) {
  .card-meta-row { display: flex; flex-direction: column-reverse; justify-content: flex-start; align-content: flex-start; align-items: normal; }
  .card-meta-chefe { display: none; }
  .receitas-grid { grid-template-columns: 1fr 1fr !important; gap: 5px; }
  .card-badge-categoria { display: none; }
  .card-image { min-height: 100px !important; }
  .card-content { padding: 10px 10px 12px; }
  .card-titulo { font-size: 0.75rem; margin-bottom: 6px; }
  .card-meta-item { font-size: 0.7rem; }
  .card-link { font-size: 0.62rem; padding: 7px 8px; }
...
@media (max-width: 900px) {
  .nova-main-layout { flex-direction: column-reverse; gap: 25px; }
  .nova-info-card { position: relative; top: 0; max-width: 100%; width: 100%; }
  .nova-form-container { width: 100%; }
}

@media (max-width: 768px) {
  .detalhe-topbar { height: 56px; padding: 0 15px; gap: 12px; }
  .detalhe-topbar-back { padding: 6px 16px; font-size: 0.75rem; gap: 6px; box-shadow: 2px 2px 0 rgba(139, 0, 0, 0.15); }
  .detalhe-topbar-back-text-full { display: none; }
  .detalhe-topbar-title { display: none; }
  .detalhe-container { padding: 65px 5px 5px; gap: 20px; }
  .nova-recipe-form { padding: 24px 20px; border: none; border-radius: 4px; box-shadow: 3px 3px 0 rgba(139, 0, 0, 0.15); }
  .nova-info-card { box-shadow: 3px 3px 0 rgba(139, 0, 0, 0.15); }
  .nova-form-row { flex-direction: column; gap: 0; }
  .nova-form-group { margin-bottom: 18px; }
  .nova-form-group label { font-size: 0.8rem; }
  .nova-form-group input, .nova-form-group select { padding: 11px 14px; font-size: 0.9rem; }
  .nova-ingrediente-row { flex-direction: column; align-items: stretch; gap: 8px; padding: 12px; background: rgba(212, 175, 55, 0.04); border-radius: 4px; border: 1px solid rgba(139, 0, 0, 0.1); }
  .nova-quantidade, .nova-unidade, .nova-ingrediente-nome-wrapper { width: 100%; min-width: 0; }
  .nova-remove-btn { width: 30px; height: 30px; font-size: 15px; }
  .nova-submit-button { padding: 14px; font-size: 0.9rem; }
  .nova-info-header { padding: 16px; }
  .nova-info-header h3 { font-size: 1rem; }
  .nova-info-body { padding: 16px; }
  .nova-info-section { margin-bottom: 18px; padding-bottom: 12px; }
  .nova-info-section h4 { font-size: 0.78rem; }
  .nova-info-section p, .nova-info-section li { font-size: 0.85rem; }
}

@media (max-width: 480px) {
  .detalhe-topbar { height: 54px; padding: 0 12px; gap: 10px; }
  .detalhe-topbar-title { font-size: 0.75rem; }
  .nova-recipe-form { padding: 20px 14px; }
  ...
  .nova-ingrediente-row { padding: 10px; }
  .nova-quantidade, .nova-unidade, .nova-ingrediente-nome { padding: 10px 12px; font-size: 0.85rem; }
...
@media (max-width: 992px) {
  ...
  .detalhe-info-actions { top: 20px; right: 30px; }
...
@media (max-width: 768px) {
  ...
  .detalhe-topbar { height: 56px; padding: 0 15px; gap: 12px; justify-content: space-between; }
  .detalhe-topbar-back { padding: 6px 16px; font-size: 0.75rem; gap: 6px; box-shadow: 2px 2px 0 rgba(139, 0, 0, 0.15); }
  .detalhe-topbar-back-text-full { display: none; }
  .detalhe-topbar-title { font-size: 0.85rem; }
  ...
  .detalhe-info-header { flex-direction: column; align-items: stretch; gap: 14px; display: flex; }
  ...
  .detalhe-ingrediente-check { width: 20px; height: 20px; }
...
@media (max-width: 480px) {
  ...
  .detalhe-badge-categoria { font-size: 0.65rem; padding: 5px 12px; }
...
@media print {
  .detalhe-topbar, .detalhe-badge-categoria, .admin-sidebar, .header-full-width, .newspaper-footer, .modal-overlay, .modal-content { display: none !important; }
...
@media (max-width: 996px) {
  ...
  .header-full-width.scrolled .newspaper-title::before, .header-full-width.scrolled .newspaper-title::after { font-size: 1.4rem; }
}
...
@media (max-width: 768px) {
  ...
  .header-nav a.active i { color: var(--header-color); }
  ...
  .header-full-width.scrolled .header-nav.open { transform: translateX(0) !important; }
  ...
  .header-full-width.scrolled .newspaper-title::before, .header-full-width.scrolled .newspaper-title::after { display: none; }
  .header-full-width.scrolled .newspaper-date, .header-full-width.scrolled .newspaper-price { display: none; }
}

@media (min-width: 900px) and (max-width: 1200px) {
  .header-full-width.scrolled .newspaper-title { font-size: 1.5rem; }
  .newspaper-title::before, .newspaper-title::after { display: none; }
}

@media (min-width: 769px) {
  .header-full-width.scrolled .header-nav.open .header-user-mobile, .header-full-width.scrolled .header-nav.open .header-notification-sidebar, .header-full-width.scrolled .header-nav.open .header-logout-sidebar, .header-full-width.scrolled .header-nav.open .header-notification-nav { display: none !important; }
}
...
@media (max-width: 768px) {
  ...
  .admin-recipe-tabs { align-items: stretch; gap: 6px; margin-bottom: 15px; overflow-x: visible; }
  .admin-recipe-tab { min-width: 0; flex: none; padding: 8px 10px; font-size: 0.65rem; }
  .admin-rejection-reason { font-size: 0.78rem; padding: 10px 12px; }
  .admin-cards-grid { grid-template-columns: 1fr; gap: 20px; }
...
@media (max-width: 768px) {
  ...
  .minha-imagem img { transition: none; }
  .btn-ver:hover, .btn-motivo:hover, .btn-excluir:hover { transform: none; }
  .detalhe-topbar-back:hover { background: transparent; color: white; border-color: rgba(255, 255, 255, 0.25); transform: none; box-shadow: 3px 3px 0 rgba(139, 0, 0, 0.15); }
  ...
  .header-nav a:hover { border-color: 2px solid var(--header-color);transform: none; box-shadow: none; }
